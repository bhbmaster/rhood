...
# NOTE: when these compressed files are analyzed their dates are written in format yyyy-mm-dd HH:MM:## as that is all we keep in the filename
CONCAT="";
OLD_INPUT_DATE=""
...
# NOTE: when these uncompressed files are analyzed their dates are written in format yyyy-mm-dd HH:MM:SS from stat output
