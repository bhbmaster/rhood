...
def PRINT_ALL_PROFILE_AND_ORDERS(save_bool=False,load_bool=False, extra_info_bool=False, csv_bool=False, csv_profile_bool=False, info_type="ALL", sort_alpha_bool=False):
    ...
    if info_type == "ALL" or info_type == "PROFILE":
        ...
        for prof in prof_type:
            ...
            if csv_profile_bool:
                print_to_csv(f"A-{prof}-profile",prof_dictionary,run_date_orders,dir_suffix,user_string)
    ...
    if info_type == "ALL" or info_type == "FINANCE":
        if csv_bool:
            if stock_orders != []:
                ...
                print_all_stocks_to_csv(stock_orders,run_date_orders,dir_suffix,user_string)
                print_to_csv("All-Profits-Stocks",list_dict_of_stock_profits,run_date_orders,dir_suffix,user_string)
                if sod != []:
                    print_to_csv("All-Open-Stocks",sod,run_date_orders,dir_suffix,user_string)
                print(f"* saved stock csvs")
            if crypto_orders != []:
                print(f"* saving crypto orders + open positions + net profits csvs to '{dir_full}' directory")
                print_all_crypto_to_csv(crypto_orders,run_date_orders,dir_suffix,user_string,cryptopairs)
                print_to_csv("All-Profits-Crypto",list_dict_of_crypto_profits,run_date_orders,dir_suffix,user_string)
                if cod != []:
                    print_to_csv("All-Open-Crypto",cod,run_date_orders,dir_suffix,user_string)
                print(f"* saved crypto csvs")
            if option_orders != []:
                print(f"* saving option orders + open positions + net profits csvs to '{dir_full}' directory")
                print_all_options_to_csv(option_orders,run_date_orders,dir_suffix,user_string)
                print_to_csv("All-Profits-Options",list_dict_of_option_profits,run_date_orders,dir_suffix,user_string)
                if ood != []:
                    print_to_csv("All-Open-Options",ood,run_date_orders,dir_suffix,user_string)
                print(f"* saved option csvs")
            if rs_divs_list != []:
                print(f"* saving dividends csv to '{dir_full}'")
                print_to_csv("All-Dividends",rs_divs_list,run_date_orders,dir_suffix,user_string)

        # Save api data to pickle file dat.pkl so we can use --load in future to load faster (but of course then its not live data)
        if save_bool:
            save_data(FILENAME, so = stock_orders, co = crypto_orders, oo = option_orders, sd = stocks_dict, cd = cryptos_dict, od = options_dict, soo = stocks_open, coo = cryptos_open, ooo = options_open, sod = sod, cod = cod, ood = ood, divs = rs_divs_list, verify_bool = True, run_date=run_date, loaded_username=loaded_username)
